...
public class BoardWriteServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(!ServletFileUpload.isMultipartContent(request)) {
			...
			request.setAttribute("msg", "자유게시물 작성오류[enctype]");
		...
		Notice n = new Notice();
		n.setNoticeTitle(mRequest.getParameter("boardTitle"));
		n.setNoticeWriter(mRequest.getParameter("boardWriter"));
		n.setNoticContent(mRequest.getParameter("boardContent"));
		...
		RequestDispatcher rd = request.getRequestDispatcher("/WEB-INF/views/common/msg.jsp");
		if(result>0) {
			request.setAttribute("msg","자유게시물 등록 성공");
		}else {
			request.setAttribute("msg","자유게시물 등록 실패");
		}
